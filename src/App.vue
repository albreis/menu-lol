<template>
  <div id="app">
    <section class="campeoes">
      <div class="header">
        <div class="headline">ESCOLHA SEU</div>
        <div class="title">CAMPEÃO</div>
        <div class="description">Quer você goste de se jogar na batalha, ajudar seus aliados ou os dois, existe um lugar para você no Rift.</div>
      </div>
      <div class="cta">
        <a href="#">Descubra mais Campeões</a>
        <a href="#">Jogar agora</a>
      </div>
      <div class="characters" :data-active="active">
        <div class="left">
          <template v-for="(char, i) in characters">
            <div class="char" :key="i" :class="{active: i == active}" @click="active = i">
              <div class="icon" v-html="char.icon"></div>
              <span>{{char.group}}</span>
              <div class="marker" v-html="marker"></div>
              <div class="dot">
                <span>
                  <span></span>
                </span>
...
export default {
  ...
  methods: {
    calcLeft() {
      let left = document.querySelector('.active').offsetLeft
      let width = document.querySelector('.active').offsetWidth
      console.log(left, width)
      this.$refs.dot.style.left = ((left + (width/2)) - 10) + 'px'
    }
  },
  data() {
    return {
...
.characters {
  ...
  .left {
    ...
    .dot-active {
      border: 1px solid rgb(195, 144, 49);
      width: 20px;
      height: 20px;
      border-radius: 100px;
      display: flex;
